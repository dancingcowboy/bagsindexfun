...
/**
 * Scoring worker — runs daily.
 * Runs three scoring passes (CONSERVATIVE / BALANCED / DEGEN), each with its
 * own universe filters and weights from TIER_SCORING_CONFIG. A Layer-A agent
 * safety review can remove tokens (but never add/reorder) before final ranking.
 */
async function processScoring(job: Job) {
  const logger = { info: console.log, error: console.error }
  logger.info(`[scoring] Starting scoring cycle (job: ${job.id})`)
