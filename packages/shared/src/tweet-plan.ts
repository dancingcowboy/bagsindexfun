...
const CA = 'DTp6oMA51WydSAcqY8cgYCFTtQXcQHNq5geCSgrwBAGS'

export const TWEET_PLAN: PlannedTweet[] = [
  // ─── Day 1 — Launch ────────────────────────────────────────────────────────
  { text: `gm. introducing @bagsindex — the first auto-rebalancing index fund for the Bags ecosystem. one deposit. top 10 tokens. zero work. ${X}`, imageQuery: 'sunrise mountain' },
  { text: `the pitch in one line:\n\nyou deposit SOL → we auto-allocate across the top Bags tokens → AI-reviewed, rebalanced daily → you sleep.\n\nno picking. no charts. no FOMO.`, imageQuery: 'minimalist desk' },
  ...
  { text: `"how do you actually buy the tokens?"\n\nwe use Bags' native /trade/swap endpoint. it returns a base58 versioned tx. Privy signs it. we broadcast. that's the entire path.\n\nno DEX aggregator middleman.`, imageQuery: 'pipeline flow industrial' },
  ...
  { text: `power user feature: you can split deposits across tiers.\n\nput 60% in conservative, 30% in balanced, 10% in degen. each tier its own sub-wallet, its own rebalance cadence.\n\nportfolio sleeves, on autopilot.`, imageQuery: 'split fork' },
  ...
  { text: `coming eventually: governance over scoring weights.\n\n$BAGSX holders can vote on whether volume should be 50% or 40% of the score. fully on-chain.\n\nuntil then, the formula is fixed and public.`, imageQuery: 'voting ballot' },
  ...
  { text: `if you're a Bags project founder reading this — go to ${X}/projects and add a vault to your fee-share.\n\na slice of your trading fees, on-chain, into a diversified ecosystem vault. zero ongoing work.`, imageQuery: 'partnership handshake' },
  ...
  { text: `if you're a security researcher — there's no formal bug bounty yet, but report a real issue privately and you'll get a meaningful $BAGSX allocation from the protocol vault. and a beer.`, imageQuery: 'shield security' },
  { text: `if you just want to lurk and watch — that's also fine. follow @bagsindex, watch the leaderboard at ${X}/projects, see if the conservative tier ever has a red day.\n\nno pressure. it's a long road.`, imageQuery: 'observer telescope' },
